...
const TextBox = (props: TextBoxProps) => {
  const inputRef = useRef<TextInput>(null); // Going to be used as a reference for the textInput, orginally set to null
  const valueRef = useRef(""); // Used to update the value inside of textInput

  const handleSubmit = () => {
    const value = valueRef.current.trim();
    if (!value) return;

    props.AnswerCallBack(value);
  };
    ...
    <View className="items-center" style={{ width: width, height: height }}>
      ...
      >
        <TextInput
          ...
          returnKeyType="done"
        />
        {!isFocused && valueRef.current.trim().length < 1 && (
          <Text
